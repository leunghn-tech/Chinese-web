...
export default function TeamMonsterActivity({ unit, subject, onBack, onComplete }) {
  ...
  const answer = (choice) => { if (feedback) return; const isCorrect = choice === question.answer; setAttempts((value) => value + 1); if (isCorrect) { const nextStreak = streak + 1; const kind = nextStreak >= battleSettings.ultimateStreak ? 'ultimate' : nextStreak === battleSettings.skillStreak ? 'skill' : null; const source = kind === 'ultimate' ? monster.ultimate : kind === 'skill' ? monster.skill : null; const bonus = kind === 'ultimate' ? battleSettings.ultimateBonusDamage : kind === 'skill' ? battleSettings.skillBonusDamage : 0; const damage = battleSettings.baseDamage + bonus; const attack = source ? { ...source, kind, threshold: nextStreak, damage } : null; setCorrect((value) => value + 1); setTeamPower((value) => value + (kind === 'ultimate' ? 50 : kind === 'skill' ? 25 : 10)); setMonsterHp((value) => Math.max(0, value - damage)); setStreak(kind === 'ultimate' ? 0 : nextStreak); setSpecialAttack(attack); if (attack) celebrate(kind); setFeedback({ correct: true, choice, damage, special: attack }); } else { setStreak(0); setSpecialAttack(null); setFeedback({ correct: false, choice }); } };
  const retry = () => { setFeedback(null); setSpecialAttack(null); setChoiceRound((value) => value + 1); };
  const next = () => { if (index === questions.length - 1) { pauseExamTimer(); onComplete?.(unit, questions.map((item) => item.id)); setShowSummary(true); return; } setIndex((value) => value + 1); setFeedback(null); setSpecialAttack(null); setChoiceRound((value) => value + 1); };
  const replay = () => { setQuestions(shuffle(unit.questions.map(asBattleQuestion).filter(Boolean)).slice(0, 10)); setIndex(0); setFeedback(null); setAttempts(0); setCorrect(0); setMonsterHp(maxHp); setTeamPower(0); setMonster(pickTeamMonster()); setStreak(0); setSpecialAttack(null); setChoiceRound(0); setShowSummary(false); };
  ...
  if (showSummary) return <main className="site-shell math-activity-page team-battle-page"><BattleHeader unit={unit} subject={subject} onBack={onBack} summary /><section className="math-activity-stage"><TeamMonsterPanel current={questions.length} total={questions.length} monsterHp={monsterHp} monsterMaxHp={maxHp} teamPower={teamPower} monster={monster} status="hit" battleSettings={battleSettings} /><UnitResultSummary unit={unit} total={questions.length} correct={correct} attempts={attempts} onBack={onBack} onReplay={replay} title={monsterHp === 0 ? `${monster.name}已被全班擊退！` : '全班合作任務完成！'} description="每一題都由同學一起商量並完成。教師可按正確率安排重試或開啟下一個互動單元。" noun="題" backLabel={`返回${subject}目錄`} /></section></main>;
  return <main className="site-shell math-activity-page team-battle-page"><BattleHeader unit={unit} subject={subject} current={index + 1} total={questions.length} onBack={onBack} /><section className="math-activity-stage team-battle-stage"><TeamMonsterPanel current={index + 1} total={questions.length} monsterHp={monsterHp} monsterMaxHp={maxHp} teamPower={teamPower} streak={streak} specialAttack={specialAttack} monster={monster} status={status} battleSettings={battleSettings} onPreviewAttack={celebrate} /><section className="team-battle-question"><span><Swords size={17} /> 全班合力題目</span><h1>{question.prompt}</h1><p>先舉手說出想法，全班確認後再由老師或同學選答案。</p><div className="math-option-grid team-battle-options">{choices.map((choice, choiceIndex) => <button key={String(choice)} disabled={Boolean(feedback)} onClick={() => answer(choice)} className={feedback && feedback.choice === choice ? feedback.correct ? 'selected-correct' : 'selected-wrong' : ''}><span>{String.fromCharCode(65 + choiceIndex)}</span><b>{choice}</b></button>)}</div></section>{feedback ? <section className={`math-feedback team-battle-feedback ${feedback.correct ? 'correct' : 'incorrect'} ${feedback.special ? 'special-hit' : ''}`} role="status"><div>{feedback.correct ? <Check size={22} /> : <X size={22} />}</div><section><b>{feedback.special ? `${feedback.special.title}發動！${monster.name}受到 ${feedback.damage} 點傷害。` : feedback.correct ? `${monster.name}被全班知識光束擊中！` : `${monster.name}正在防守，先看看提示再合力一次。`}</b><p>{feedback.special ? feedback.special.message : feedback.correct ? question.explanation : <>正確答案是 <strong>{question.answer}</strong>。{question.explanation}</>}</p><div className="complete-actions">{feedback.correct ? <button onClick={next}>{index === questions.length - 1 ? '查看合作結算' : '下一題・繼續合力'} <ChevronRight size={17} /></button> : <button onClick={retry}><RotateCcw size={16} /> 重新商量再試</button>}<button onClick={onBack}><ArrowLeft size={16} /> 返回目錄</button></div></section></section> : null}</section></main>;
}
