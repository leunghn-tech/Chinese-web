...
export default function SentenceExpandActivity({ unit, onBack, onComplete }) {
  ...
  const resetQuestion = () => { setAnswers({}); setFeedback(null); setShuffleRound((round) => round + 1); };
  ...
  const nextQuestion = () => {
    if (questionIndex >= questions.length - 1) { onComplete?.(unit); setShowSummary(true); return; }
    setQuestionIndex((index) => index + 1);
    resetQuestion();
  };

  if (showSummary) return <main className="site-shell sentence-expand-page"><WorkbenchFrame unit={unit} taskLabel="結算" /><section className="activity-summary writing-summary"><span><Trophy size={28} /> 寫作任務完成</span><h1>你已組成全部<br /><em>{questions.length} 個完整句子。</em></h1><p>你已練習在句子中清楚交代時間、人物、地點和動作。</p><div className="summary-score"><b>{questions.length}</b><span>題全部完成</span></div><div className="summary-actions"><button onClick={onBack}>返回中文目錄</button><button onClick={replay}><RotateCcw size={17} /> 隨機重玩</button></div></section></main>;
