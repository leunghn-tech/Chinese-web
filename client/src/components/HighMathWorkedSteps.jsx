...
export default function HighMathWorkedSteps({ unit, question }) {
  ...
  return <section className="worked-steps" aria-label="圖像化解題步驟"><header><span><Lightbulb size={17} /> 圖像化解題步驟</span><small>{plan.tag}</small></header><div className="worked-step-grid"><article><b><ScanSearch size={18} /> 1. 看甚麼？</b><p><strong>{plan.key}</strong>{plan.note}</p></article><article className="worked-model"><b>2. 用模型想一想</b><StrategyModel plan={plan} question={question} /></article><article><b><CheckCircle2 size={18} /> 3. 列式與檢查</b><p>{plan.formula}</p><em>{question.explanation}</em></article></div></section>;
}
