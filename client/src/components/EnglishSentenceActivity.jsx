...
export default function EnglishSentenceActivity({ unit, onBack, onComplete }) {
  ...
  const resetQuestion = () => { setHeard(false); setSelectedTokens([]); setDraggedId(null); setDropPulse(false); setHintUsed(false); setFeedback(null); setShuffleRound((round) => round + 1); };
  const goNext = () => {
  ...
  const replay = () => { setQuestions(shuffle(unit.questions)); setQuestionIndex(0); setHeard(false); setSelectedTokens([]); setDraggedId(null); setDropPulse(false); setHintUsed(false); setFeedback(null); setShuffleRound(0); setShowSummary(false); setAttempts(0); setCorrectCount(0); };
  const checkSentence = () => {
