...
export default function TeamMonsterMathActivity({ unit, onBack, onComplete }) {
  ...
  const answer = (choice) => { if (feedback) return; const isCorrect = choice === question.answer; setAttempts((value) => value + 1); if (isCorrect) { const nextStreak = streak + 1; const kind = nextStreak >= 5 ? 'ultimate' : nextStreak === 3 ? 'skill' : null; const source = kind === 'ultimate' ? monster.ultimate : kind === 'skill' ? monster.skill : null; const damage = source ? 10 + source.bonusDamage : 10; const attack = source ? { ...source, kind, threshold: nextStreak, damage } : null; setCorrect((value) => value + 1); setTeamPower((value) => value + (kind === 'ultimate' ? 50 : kind === 'skill' ? 25 : 10)); setMonsterHp((value) => Math.max(0, value - damage)); setStreak(kind === 'ultimate' ? 0 : nextStreak); setSpecialAttack(attack); setFeedback({ correct: true, choice, damage, special: attack }); } else { setStreak(0); setSpecialAttack(null); setFeedback({ correct: false, choice }); } };
  ...
  const replay = () => { setQuestions(shuffle(unit.questions)); setIndex(0); setFeedback(null); setAttempts(0); setCorrect(0); setMonsterHp(unit.questions.length * 10); setTeamPower(0); setMonster(pickTeamMonster()); setStreak(0); setSpecialAttack(null); setChoiceRound(0); setShowSummary(false); };
