...
export default function TeamMonsterMathActivity({ unit, onBack, onComplete }) {
  ...
  const answer = (choice) => { if (feedback) return; const isCorrect = choice === question.answer; setAttempts((value) => value + 1); if (isCorrect) { const nextStreak = streak + 1; const isSpecial = nextStreak >= 3; const damage = isSpecial ? 10 + monster.skill.bonusDamage : 10; const attack = isSpecial ? { ...monster.skill, damage } : null; setCorrect((value) => value + 1); setTeamPower((value) => value + (isSpecial ? 25 : 10)); setMonsterHp((value) => Math.max(0, value - damage)); setStreak(isSpecial ? 0 : nextStreak); setSpecialAttack(attack); setFeedback({ correct: true, choice, damage, special: attack }); } else { setStreak(0); setSpecialAttack(null); setFeedback({ correct: false, choice }); } };
