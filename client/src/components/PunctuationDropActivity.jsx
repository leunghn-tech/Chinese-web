...
export default function PunctuationDropActivity({ unit, onBack, onComplete }) {
  ...
  const resetQuestion = () => { setSelectedMark(null); setPlacedMark(null); setFeedback(null); setShuffleRound((round) => round + 1); };
  const replay = () => { setQuestionIndex(Math.floor(Math.random() * unit.questions.length)); setShowSummary(false); resetQuestion(); };
