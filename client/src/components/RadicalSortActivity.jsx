...
export default function RadicalSortActivity({ unit, onBack, onComplete }) {
  ...
  if (showSummary) return <main className="site-shell radical-page"><section className="activity-summary"><span><Trophy size={28} /> 任務完成</span><h1>你已完成全部<br /><em>{unit.questions.length} 題部首任務。</em></h1><p>你已練習從字形找出常用部首。可以重玩一次，或返回中文目錄選擇下一個單元。</p><div className="summary-score"><b>{unit.questions.length}</b><span>題全部完成</span></div><div className="summary-actions"><button onClick={onBack}>返回中文目錄</button><button onClick={replay}><RotateCcw size={17} /> 重玩這個單元</button></div></section></main>;
