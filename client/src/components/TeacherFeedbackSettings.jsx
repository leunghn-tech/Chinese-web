...
const DEFAULTS = { sound: true, animation: true, hintSatchel: false, firstWordHint: false, battlePreset: 'standard', battle: BATTLE_DEFAULTS };
...
export default function TeacherFeedbackSettings() {
  ...
  const applyPreset = (preset) => { const next = { ...settings, battlePreset: preset.id, battle: { ...preset.battle } }; setSettings(next); saveSettings(next); };
  const updateBattle = (key, rawValue) => {
    ...
    const next = { ...settings, battlePreset: 'custom', battle };
    setSettings(next);
    saveSettings(next);
  };
