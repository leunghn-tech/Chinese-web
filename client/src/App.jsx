...
function QuestionBankStatus({ questionBank, questionCount, completedUnits, onStartUnit }) {
  ...
  return <section className="question-bank-status"><div className="bank-overview"><div><span>本級試題庫</span><b>{readyCount} 個單元可開始</b><small>共 {questionBank.units.length} 個單元入口，已加入 {questionCount} 題。</small></div><strong className="bank-total-chip">{questionCount} 題</strong></div><div className="bank-unit-grid">{questionBank.units.map((unit) => { const ready = unit.questions.length > 0; const completed = Math.min(completedUnits[unit.id] || 0, unit.questions.length); const percent = unit.questions.length ? Math.round((completed / unit.questions.length) * 100) : 0; const content = <><div className="bank-unit-card-top"><span>{unit.area}</span><b className="bank-unit-count">{unit.questions.length} 題</b></div><strong>{unit.title}</strong><div className="bank-unit-progress"><span>{ready ? `${completed} / ${unit.questions.length} 題已完成` : '題庫準備中'}</span><i><b style={{ width: `${percent}%` }} /></i></div></>; return ready ? <button key={unit.id} className="bank-unit-card ready" onClick={() => onStartUnit(unit)}>{content}<ChevronRight size={18} /></button> : <article key={unit.id} className="bank-unit-card pending">{content}</article>; })}</div></section>;
}
