...
function Home({ onStart, onCatalog }) {
  return <main className="site-shell home-page"><Header /><div className="floaters" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div><section className="home-hero"><div className="hero-copy"><span className="kicker"><Sparkles size={15} /> EDUQUEST・暫定名稱</span><h1>為下一堂課，<em>留下清楚的起點。</em></h1><p>先建立小一至小六的中、英、數課程框架。中文課程目錄現已按閱讀與寫作分級整理；英文和數學暫保留一題示範，待你逐科補上內容。</p><div className="hero-actions"><button className="primary-button" onClick={onStart}><Play size={18} fill="currentColor" /> 開始選年級與學科 <ChevronRight size={18} /></button><span>先看目錄，再安排下一步。</span></div></div><aside className="launch-desk"><div className="desk-heading"><span>今天的開課檯</span><b>三步設定</b></div><ol><li><b>01</b><div><strong>選擇年級</strong><small>P1 至 P6</small></div></li><li><b>02</b><div><strong>選擇學科</strong><small>中文、英文、數學</small></div></li><li><b>03</b><div><strong>試玩示範題</strong><small>每科暫設一題</small></div></li></ol><div className="desk-note"><CircleHelp size={17} /><span>中文目錄已按分級重點整理。</span></div></aside></section><section className="class-start-strip" aria-label="今天課堂的開課路徑"><div className="start-step"><span>1</span><div><b>先選年級</b><small>P1 至 P6</small></div></div><i></i><div className="start-step subject-step"><span>2</span><div><b>再選學科</b><small><em className="chinese-dot">中文</em><em className="english-dot">英文</em><em className="math-dot">數學</em></small></div></div><i></i><div className="start-step"><span>3</span><div><b>開啟任務卡</b><small>查看目錄或試玩一題</small></div></div></section><section className="home-stats"><article><b>6</b><span>小學年級</span></article><article><b>2</b><span>中文範疇：閱讀、寫作</span></article><article><b>18</b><span>可操作示範題</span></article></section><section className="chinese-directory-callout"><div><span className="kicker">中文課程目錄</span><h2>中文課程，<br />先看清每級重點。</h2><p>以閱讀與寫作雙軸整理小一至小六的學習重點，讓備課與後續題庫擴充有清楚依據。</p></div><div className="catalog-preview"><span>小一至小六</span><b>閱讀 × 寫作</b><small>按現時分級範疇整理</small><button onClick={onCatalog}>查看中文科目錄 <ChevronRight size={17} /></button></div></section><section className="subject-intro"><div><span className="kicker">三科課程框架</span><h2>中、英、數<br />先把位置準備好。</h2></div><div className="subject-intro-list">{Object.entries(SUBJECTS).map(([name, subject]) => { const Icon = subject.icon; return <span className={`subject-mini ${subject.color}`} key={name}><Icon size={18} /><b>{name}</b><small>{subject.english}</small></span>; })}</div></section></main>;
}
...
function CourseCard({ topic, onOpen, onCatalog }) {
  ...
  return <article className={`course-card ${subject.color}`}><div className="course-card-icon"><Icon size={27} /></div><div className="course-card-main"><span>{topic.subject}・{subject.english}</span><h3>{isChinese ? 'P1–P6 中文科目錄' : topic.title}</h3><p>{isChinese ? '閱讀與寫作分級重點已整理' : topic.description}</p></div><div className="course-card-action"><small>{isChinese ? '閱讀・寫作' : '示範題 01'}</small><button onClick={() => isChinese ? onCatalog() : onOpen(topic)}>{isChinese ? '查看目錄' : '開啟示範'} <ChevronRight size={17} /></button></div></article>;
}
...
export default function App() {
  ...
  const [screen, setScreen] = useState(previewTopic ? 'demo' : previewView === 'catalog' ? 'catalog' : previewView === 'courses' ? 'courses' : 'home');
  const [topic, setTopic] = useState(previewTopic);
  const openDemo = (selectedTopic) => { setTopic(selectedTopic); setScreen('demo'); };
  if (screen === 'catalog') return <ChineseCatalog onBack={() => setScreen('courses')} onHome={() => setScreen('home')} />;
  if (screen === 'courses') return <Courses onBack={() => setScreen('home')} onOpen={openDemo} onCatalog={() => setScreen('catalog')} />;
  if (screen === 'demo' && topic) return <Demo topic={topic} onBack={() => setScreen('courses')} />;
  return <Home onStart={() => setScreen('courses')} onCatalog={() => setScreen('catalog')} />;
}
