...
function QuestionBankStatus({ questionBank, questionCount, onStartUnit }) {
  const readyCount = questionBank.units.filter((unit) => unit.questions.length > 0).length;
  return <section className="question-bank-status"><div className="bank-overview"><div><span>本級試題庫</span><b>{readyCount} 個單元可開始</b><small>共 {questionBank.units.length} 個單元入口，已加入 {questionCount} 題。</small></div><strong className="bank-total-chip">{questionCount} 題</strong></div><div className="bank-unit-grid">{questionBank.units.map((unit) => { const ready = unit.questions.length > 0; const content = <><div className="bank-unit-card-top"><span>{unit.area}</span><b className="bank-unit-count">{unit.questions.length} 題</b></div><strong>{unit.title}</strong><small>{ready ? '開始互動練習' : '題庫準備中'}</small></>; return ready ? <button key={unit.id} className="bank-unit-card ready" onClick={() => onStartUnit(unit)}>{content}<ChevronRight size={18} /></button> : <article key={unit.id} className="bank-unit-card pending">{content}</article>; })}</div></section>;
}

function ChineseCatalog({ onBack, onHome, initialGrade, onStartUnit }) {
...
export default function App() {
  ...
  const previewUnitId = params.get('activity') === 'p1-radical-sort' ? 'P1-CN-R02' : params.get('activity') === 'p1-word-match' ? 'P1-CN-R01' : null;
  ...
  if (screen === 'activity' && activeUnit) return activeUnit.interaction === 'radical-sort' ? <RadicalSortActivity unit={activeUnit} onBack={() => setScreen('catalog')} /> : <WordMatchActivity unit={activeUnit} onBack={() => setScreen('catalog')} />;
  if (screen === 'catalog') return <ChineseCatalog initialGrade={catalogGrade} onBack={() => setScreen('courses')} onHome={() => setScreen('home')} onStartUnit={(unit) => { setActiveUnit(unit); setScreen('activity'); }} />;
