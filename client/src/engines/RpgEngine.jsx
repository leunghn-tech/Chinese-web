...
export default function RpgEngine({ question, onAnswer, disabled }) {
  return <div className="engine rpg-engine">
    ...
    <div className="question-card battle-question"><p className="question-number">KNOWLEDGE SPELL</p><h3>{question.prompt}</h3>{question.promptEn && <p className="question-translation">{question.promptEn}</p>}<div className="battle-options">{question.options.map((option, index) => <button key={option} disabled={disabled} onClick={() => onAnswer(index === question.answerIndex, option)} className="answer-btn spell-answer"><span className="spell-icon">{['✦', '◈', '✧', '⬢'][index]}</span>{option}</button>)}</div></div>
  </div>;
}
