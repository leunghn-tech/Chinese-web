...
export default function PuzzleEngine({ question, onAnswer, disabled, questionKey }) {
  ...
  return <div className="engine puzzle-engine">
    ...
    <div className="question-card"><p className="question-number">ORDER QUEST</p><h3>{question.prompt}</h3><div className="puzzle-target">{picked.length ? picked.map((item, index) => <span key={`${item.index}-${index}`}>{item.token}</span>) : <em>在下方點選碎片，建立答案。</em>}</div><div className="token-row">{question.tokens.map((token, index) => <button disabled={disabled || picked.some((item) => item.index === index)} onClick={() => choose(token, index)} className="token-btn" key={`${token}-${index}`}>{token}</button>)}</div><button className="minor-btn" disabled={disabled || !picked.length} onClick={() => setPicked([])}><RotateCcw size={15} /> 清除重排</button></div>
  </div>;
}
