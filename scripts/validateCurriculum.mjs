...
if (topics.length !== expectedTopicCount || totalQuestions !== expectedTopicCount * 15 || wrongSubjects.length || wrongGrades.length || wrongTopicShape.length || mappingErrors.length || duplicateQuestionTopics.length || genericTemplateErrors.length || radicalBuildErrors || connectorFillErrors || classicalMarkErrors || fableSortErrors || applicationMaterialErrors.length || database.availableSubjects?.join(',') !== '中文' || database.availableGrades?.join(',') !== 'P1,P2,P3,P4,P5,P6') {
  console.error({ topicCount:topics.length, totalQuestions, wrongSubjects:wrongSubjects.map((topic) => topic.id), wrongGrades:wrongGrades.map((topic) => topic.id), wrongTopicShape:wrongTopicShape.map((topic) => topic.id), mappingErrors, duplicateQuestionTopics:duplicateQuestionTopics.map((topic) => topic.id), genericTemplateErrors, radicalBuildErrors, connectorFillErrors, classicalMarkErrors, fableSortErrors, applicationMaterialErrors:applicationMaterialErrors.map((topic) => topic.id) });
  process.exit(1);
}

console.log(JSON.stringify({ status:'valid', topics:topics.length, totalQuestions, grades:database.availableGrades, subjects:database.availableSubjects, questionsPerTopic:15, radicalBuildQuestions:radicalTopic.questions.length, connectorFillQuestions:connectorTopic.questions.length, fableSortQuestions:fableTopic.questions.length, classicalMarkQuestions:classicalTopic.questions.length, applicationMaterialQuestions:topics.filter((topic) => applicationTopicTitles.has(topic.title)).reduce((total, topic) => total + topic.questions.length, 0), mappingErrors:0 }, null, 2));
